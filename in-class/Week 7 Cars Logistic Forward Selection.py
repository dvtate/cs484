# Name: Week 7 Cars Logistic Forward Selection.py
...
# Note: Since Pandas will not append a non-categorical index (as in xInt) to a categorical index (as in xCat)
#       but will append in another direction, so we append xCat to xInt instead.  The model is still the same.
designX = xInt
